...
-- Create a SECURITY DEFINER function that validates prices server-side
...
BEGIN
  ...
  -- Look up delivery charge from shipping_zones
  ...
  -- Process each item: look up real prices from DB
  ...
  LOOP
    ...
    -- If variant specified, apply price adjustment
    IF v_item->>'variant_label' IS NOT NULL AND (v_item->>'variant_label') != '' THEN
      SELECT * INTO v_variant
      FROM public.product_variants
      WHERE product_id = v_product.id
        AND variant_label = (v_item->>'variant_label')
        AND is_active = true;
  ...
  INSERT INTO public.orders (
    ...
    'Pending', 'Pending', p_user_id, NULLIF(p_delivery_note, '')
  );
