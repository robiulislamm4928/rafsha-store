...
const AdminReviews = () => {
  ...
  const approve = async (id: string, val: boolean) => {
    ...
    toast({ title: val ? "অনুমোদিত" : "প্রত্যাখ্যাত" }); fetch();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4">
        {reviews.length === 0 ? (
          <p className="text-muted-foreground text-center py-8">কোনো রিভিউ নেই</p>
        ) : reviews.map((r) => (
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <span className={`px-2 py-0.5 rounded-full ${r.is_approved ? "bg-green-100 text-green-700" : "bg-honey-gold/20 text-honey-deep"}`}>{r.is_approved ? "অনুমোদিত" : "পেন্ডিং"}</span>
                </div>
              </div>
              <div className="flex gap-1 shrink-0 ml-3">
                {!r.is_approved && <Button variant="ghost" size="icon" className="h-7 w-7 text-green-600" onClick={() => approve(r.id, true)}><Check className="h-3.5 w-3.5" /></Button>}
