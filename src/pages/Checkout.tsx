...
import { ShoppingCart, ArrowLeft, Package, Tag, X, Copy, Check, MapPin, CreditCard, CheckCircle } from "lucide-react";
...
const steps = [
  { id: 1, label: "তথ্য", icon: MapPin },
  { id: 2, label: "পেমেন্ট", icon: CreditCard },
  { id: 3, label: "নিশ্চিত", icon: CheckCircle },
];
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    supabase.from("shipping_zones").select("delivery_charge").eq("zone_name", form.district).eq("is_active", true).maybeSingle().then(({ data }) => { setRawDeliveryCharge(data?.delivery_charge ?? 0); });
  }, [form.district]);

  useEffect(() => {
    if (freeDeliveryThreshold > 0 && total >= freeDeliveryThreshold) setDeliveryCharge(0);
    else setDeliveryCharge(rawDeliveryCharge);
  }, [total, rawDeliveryCharge, freeDeliveryThreshold]);

  useEffect(() => {
    if (!appliedCoupon) { setDiscountAmount(0); return; }
    if (appliedCoupon.discount_type === "percentage") setDiscountAmount(Math.round(total * appliedCoupon.discount_value / 100));
    else setDiscountAmount(Math.min(appliedCoupon.discount_value, total));
  }, [appliedCoupon, total]);

  const grandTotal = total + deliveryCharge - discountAmount;

  const updateField = (key: string, value: string) => { setForm((prev) => ({ ...prev, [key]: value })); setErrors((prev) => ({ ...prev, [key]: "" })); };
  ...
  const copyNumber = (num: string) => { navigator.clipboard.writeText(num); setCopiedNumber(true); toast.success("নম্বর কপি হয়েছে"); setTimeout(() => setCopiedNumber(false), 2000); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.rpc("create_order", { p_customer_name: parsed.data.customer_name, p_customer_phone: parsed.data.customer_phone, p_customer_email: parsed.data.customer_email || "", p_district: parsed.data.district, p_delivery_address: parsed.data.delivery_address, p_delivery_note: parsed.data.delivery_note || "", p_payment_method: form.payment_method === "MOBILE_BANKING" ? `MOBILE_BANKING_${mobileBankingMethod?.toUpperCase()}` : "COD", p_user_id: user?.id || "00000000-0000-0000-0000-000000000000", p_items: orderItems } as any);
      ...
      if (appliedCoupon) await supabase.from("coupons").update({ used_count: undefined as any }).eq("code", appliedCoupon.code);
      setSubmitting(false); clearCart(); toast.success("অর্ডার সফলভাবে সম্পন্ন হয়েছে!");
      navigate("/order-success", { state: { orderNumber: result.order_number, customerName: parsed.data.customer_name, customerPhone: parsed.data.customer_phone, items, subtotal: result.subtotal, deliveryCharge: result.delivery_charge, grandTotal: result.total_amount, paymentMethod: form.payment_method === "MOBILE_BANKING" ? `মোবাইল ব্যাংকিং (${mobileBankingMethod === "bkash" ? "বিকাশ" : "নগদ"})` : "ক্যাশ অন ডেলিভারি", district: parsed.data.district, address: parsed.data.delivery_address } });
    } catch (err: any) { setSubmitting(false); toast.error("অর্ডার ব্যর্থ হয়েছে: " + (err?.message || "অজানা ত্রুটি")); }
  };
        ...
        <div className="flex items-center justify-center gap-0 mb-8 max-w-md mx-auto">
          {steps.map((step, idx) => (
            <div key={step.id} className="flex items-center flex-1">
              <button onClick={() => setCurrentStep(step.id)} className={cn("flex flex-col items-center gap-1 flex-1 transition-colors", currentStep >= step.id ? "text-primary" : "text-muted-foreground")}>
                <div className={cn("w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all", currentStep > step.id ? "bg-primary border-primary text-primary-foreground" : currentStep === step.id ? "border-primary text-primary" : "border-border")}>
              ...
              {idx < steps.length - 1 && (
                <div className={cn("h-0.5 flex-1 -mt-5 mx-1 transition-colors", currentStep > step.id ? "bg-primary" : "bg-border")} />
              )}
            </div>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className={cn(currentStep === 1 ? "block" : "hidden")}>
                <div className="bg-card rounded-xl border border-border p-4 md:p-6 space-y-4">
                  ...
                  <div className="space-y-2"><Label>জেলা / শিপিং জোন *</Label>
                    <Select value={form.district} onValueChange={(v) => updateField("district", v)}><SelectTrigger><SelectValue placeholder="জোন নির্বাচন করুন" /></SelectTrigger><SelectContent className="max-h-60">{shippingZones.map((z) => <SelectItem key={z.zone_name} value={z.zone_name}>{z.zone_name} (৳{z.delivery_charge})</SelectItem>)}</SelectContent></Select>
                  ...
                  <Button type="button" onClick={() => setCurrentStep(2)} className="w-full brand-gradient text-primary-foreground hover:opacity-90 mt-2">পরবর্তী ধাপ →</Button>
                </div>
              </div>
              ...
              <div className={cn(currentStep === 2 ? "block" : "hidden")}>
                <div className="bg-card rounded-xl border border-border p-4 md:p-6 space-y-4">
                  ...
                  <RadioGroup value={form.payment_method} onValueChange={(v) => { updateField("payment_method", v); if (v === "COD") setMobileBankingMethod(null); }} className="space-y-3">
                    <div className="flex items-start space-x-3 p-3 rounded-lg border border-border hover:border-primary/50 transition-colors"><RadioGroupItem value="COD" id="cod" className="mt-0.5" /><Label htmlFor="cod" className="cursor-pointer flex-1"><span className="font-medium">ক্যাশ অন ডেলিভারি (COD)</span><p className="text-xs text-muted-foreground mt-1">পণ্য হাতে পেয়ে সম্পূর্ণ টাকা পরিশোধ করুন</p></Label></div>
                    <div className="flex items-start space-x-3 p-3 rounded-lg border border-border hover:border-primary/50 transition-colors"><RadioGroupItem value="MOBILE_BANKING" id="mobile_banking" className="mt-0.5" /><Label htmlFor="mobile_banking" className="cursor-pointer flex-1"><span className="font-medium">মোবাইল ব্যাংকিং</span><p className="text-xs text-muted-foreground mt-1">বিকাশ অথবা নগদের মাধ্যমে পেমেন্ট করুন</p></Label></div>
                  </RadioGroup>
                  ...
                  {form.payment_method === "MOBILE_BANKING" && (
                    <div className="ml-4 sm:ml-8 space-y-4 border-l-2 border-primary/20 pl-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        <button type="button" onClick={() => setMobileBankingMethod("bkash")} className={`flex-1 p-3 rounded-lg border-2 text-left transition-all ${mobileBankingMethod === "bkash" ? "border-[#E2136E] bg-[#E2136E]/5" : "border-border hover:border-[#E2136E]/50"}`}><span className="font-bold text-[#E2136E]">বিকাশ</span><p className="text-xs text-muted-foreground mt-1">বিকাশ অ্যাপ দিয়ে পেমেন্ট করুন</p></button>
                        <button type="button" onClick={() => setMobileBankingMethod("nagad")} className={`flex-1 p-3 rounded-lg border-2 text-left transition-all ${mobileBankingMethod === "nagad" ? "border-[#F6921E] bg-[#F6921E]/5" : "border-border hover:border-[#F6921E]/50"}`}><span className="font-bold text-[#F6921E]">নগদ</span><p className="text-xs text-muted-foreground mt-1">নগদ অ্যাপ দিয়ে পেমেন্ট করুন</p></button>
                      </div>
                      {mobileBankingMethod && activePaymentNumber && (
                        <div className={`rounded-lg p-4 space-y-3 ${mobileBankingMethod === "bkash" ? "bg-[#E2136E]/5 border border-[#E2136E]/20" : "bg-[#F6921E]/5 border border-[#F6921E]/20"}`}>
                          <h4 className="font-semibold text-foreground text-sm">{mobileBankingMethod === "bkash" ? "বিকাশ" : "নগদ"} পেমেন্ট নির্দেশনা:</h4>
                          ...
                          <div className="flex items-center gap-2 bg-background rounded-lg p-3 border border-border">
                            ...
                            <Button type="button" variant="outline" size="sm" onClick={() => copyNumber(activePaymentNumber)} className="shrink-0">{copiedNumber ? <Check className="h-4 w-4 text-green-600" /> : <Copy className="h-4 w-4" />}<span className="ml-1 text-xs">{copiedNumber ? "কপি হয়েছে" : "কপি"}</span></Button>
                          </div>
                        </div>
                      )}
                      {mobileBankingMethod && !activePaymentNumber && (
                        <div className="rounded-lg p-4 bg-destructive/5 border border-destructive/20 text-sm text-destructive">{mobileBankingMethod === "bkash" ? "বিকাশ" : "নগদ"} নম্বর এখনো সেট করা হয়নি।</div>
                      )}
                    </div>
                  )}
                  <div className="flex gap-3 mt-2">
                    <Button type="button" variant="outline" onClick={() => setCurrentStep(1)} className="flex-1">← আগের ধাপ</Button>
                    <Button type="button" onClick={() => setCurrentStep(3)} className="flex-1 brand-gradient text-primary-foreground hover:opacity-90">রিভিউ করুন →</Button>
                  </div>
                </div>
              </div>
              ...
              <div className={cn(currentStep === 3 ? "block" : "hidden")}>
                <div className="bg-card rounded-xl border border-border p-4 md:p-6 space-y-4">
                  <h2 className="font-display font-bold text-lg text-foreground">অর্ডার রিভিউ</h2>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div><p className="text-muted-foreground">নাম</p><p className="font-medium text-foreground">{form.customer_name || "—"}</p></div>
                    <div><p className="text-muted-foreground">ফোন</p><p className="font-medium text-foreground">{form.customer_phone || "—"}</p></div>
                    <div><p className="text-muted-foreground">জেলা</p><p className="font-medium text-foreground">{form.district || "—"}</p></div>
                    <div><p className="text-muted-foreground">পেমেন্ট</p><p className="font-medium text-foreground">{form.payment_method === "COD" ? "ক্যাশ অন ডেলিভারি" : `মোবাইল ব্যাংকিং (${mobileBankingMethod === "bkash" ? "বিকাশ" : "নগদ"})`}</p></div>
                  </div>
                  <p className="text-sm text-muted-foreground">{form.delivery_address || "—"}</p>
                  <div className="flex gap-3 mt-2">
                    <Button type="button" variant="outline" onClick={() => setCurrentStep(2)} className="flex-1">← পেমেন্ট</Button>
                    <Button type="submit" className="flex-1 brand-gradient text-primary-foreground font-semibold shadow-lg hover:opacity-90" disabled={submitting}>{submitting ? "অর্ডার হচ্ছে..." : "অর্ডার নিশ্চিত করুন"}</Button>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-card rounded-xl border border-border p-4 md:p-6 sticky top-24 space-y-4">
                ...
                <div className="border-t border-border pt-3">
                  {appliedCoupon ? (
                    <div className="flex items-center justify-between bg-primary/5 rounded-lg px-3 py-2">
                      <div className="flex items-center gap-2"><Tag className="h-4 w-4 text-primary" /><span className="text-sm font-medium text-primary">{appliedCoupon.code}</span><span className="text-xs text-muted-foreground">(-৳{discountAmount})</span></div>
                    ...
                    <div className="flex gap-2"><Input value={couponCode} onChange={(e) => setCouponCode(e.target.value)} placeholder="কুপন কোড" className="text-sm" /><Button type="button" variant="outline" size="sm" onClick={applyCoupon} disabled={couponLoading} className="shrink-0">{couponLoading ? "..." : "প্রয়োগ"}</Button></div>
                  )}
                </div>
                <div className="border-t border-border pt-3 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-foreground/80">
                    ...
                    <span>{!form.district ? "—" : freeDeliveryThreshold > 0 && total >= freeDeliveryThreshold ? <span className="text-green-600 font-medium">ফ্রি! <span className="line-through text-muted-foreground font-normal">৳{rawDeliveryCharge}</span></span> : `৳${deliveryCharge}`}</span>
                  </div>
                  {freeDeliveryThreshold > 0 && total < freeDeliveryThreshold && form.district && <p className="text-xs text-primary">আরও ৳{freeDeliveryThreshold - total} অর্ডার করলে ডেলিভারি ফ্রি!</p>}
